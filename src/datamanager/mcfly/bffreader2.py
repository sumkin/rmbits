            ...
            if child.tag == 'previous':
                ...
                for pchild in child.getchildren():
                    ...
                    for ppchild in pchild.getchildren():
                        ...
                        try:
                            ampc = ppchild.attrib['adjusted_margianl_profit_curve']
                        except:
                            ampc = smpc
        ...
        for gts in gtss:
            ...
            for fcst in fcsts:
                ...
                row = [base_orig,base_dstn,base_via,\
                       ...
                       sfd,srd,srgccd,sgcd,srgcd,\
                       afd,ard,argccd,agcd,argcd,\
        ...
        for gts in l[3][2]:
            ...
            for fcst in fcsts:      
                ...
                for i in range(len(dcps)):
                    ...
                    row = brow + [fcst[0],fcst[1],fcst[2],fcst[7]] +\
                                 [smpcs[i],ampcs[i],srdcs[i]/numdays,sgcdcs[i]/numdays,ardcs[i]/numdays,agcdcs[i]/numdays,src_date_s]
...
if __name__ == "__main__":
    xmlfname = '/home/ay49514/tmp/new_bff.xml'
    s3copy('s3://ay-dp-prod-data-inbound-nrm/'+\
           'uncompressed/boomi/bff/yyyy=2019/mm=11/dd=07/'+\
           'PRD.RMS.NFS.XML.D191104.T060002.AMA.FIN.DD200507.xml.gz', xmlfname+'.gz')
    subprocess.check_output(['gunzip',xmlfname+'.gz'])
    csvoutfname = '/home/ay49514/tmp/FCST_OD_DCP.csv'
    ...
    with open(csvoutfname, 'w') as fout:
        ...
        for bts in bffReader.btss():
            for csvline in BFFReader2.get_dcp_csv_lines(bts, '20190222', ap):
